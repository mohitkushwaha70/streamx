...
// Route: /stream/hf/{encodedFullUrl} — proxy from allowed HF repos only
const ALLOWED_HF_HOSTS = ['huggingface.co', 'hf.co'];
router.get('/hf/*', async (req, res) => {
  ...
  try {
    ...
    if (!targetUrl.startsWith('https://')) return res.status(400).send('Invalid URL');

    // Whitelist: only allow HuggingFace hosts
    const urlHost = new URL(targetUrl).hostname;
    if (!ALLOWED_HF_HOSTS.some(h => urlHost === h || urlHost.endsWith('.' + h))) {
