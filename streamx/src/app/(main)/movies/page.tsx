...
async function getMovies(params: { genre?: string; sort?: string; page?: string }) {
  const page = parseInt(params.page || '1');
  const limit = 24;
  const skip = (page - 1) * limit;

  const where: Prisma.ContentWhereInput = { published: true, type: 'MOVIE' };
...
async function MoviesGrid({ genre, sort, page }: { genre?: string; sort?: string; page?: string }) {
  const data = await getMovies({ genre, sort, page });
      ...
      {data.pages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          {Array.from({ length: Math.min(data.pages, 10) }, (_, i) => i + 1).map((p) => (
            <a
              key={p}
              href={`/movies?genre=${genre || ''}&sort=${sort || ''}&page=${p}`}
    ...
    <div className="min-h-screen pt-20">
      <div className="mx-auto max-w-[1440px] px-4 lg:px-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted">Browse our collection of movies</p>
        </div>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-4">
          <div className="flex flex-wrap gap-1.5">
            ...
            {GENRES.map((g) => (
              <a
                key={g}
                href={`/movies?genre=${encodeURIComponent(g)}&sort=${sort}`}
                className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
                  genre === g
                    ? 'bg-accent text-white'
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              defaultValue={sort}
              onChange={undefined}
              className="rounded-md border border-border bg-surface px-2.5 py-1.5 text-xs text-white outline-none focus:border-accent"
            >
        ...
        >
          <MoviesGrid genre={genre} sort={sort} page={params.page} />
        </Suspense>
      </div>
    </div>
  );
