...
const GENRES = [
  ...
  'Slice of Life', 'Sports', 'Supernatural', 'Mecha', 'Mystery', 'Horror',
];
...
async function getAnime(params: { genre?: string; sort?: string; page?: string }) {
  const page = parseInt(params.page || '1');
  const limit = 24;
  const skip = (page - 1) * limit;

  const where: Prisma.ContentWhereInput = { published: true, type: 'ANIME' };
...
async function AnimeGrid({ genre, sort, page }: { genre?: string; sort?: string; page?: string }) {
  const data = await getAnime({ genre, sort, page });

  if (data.items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <p className="text-lg font-medium text-white">No anime found</p>
      ...
      {data.pages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          {Array.from({ length: Math.min(data.pages, 10) }, (_, i) => i + 1).map((p) => (
            <a
              key={p}
              href={`/anime?genre=${genre || ''}&sort=${sort || ''}&page=${p}`}
  ...
  return (
    <div className="min-h-screen pt-20">
      <div className="mx-auto max-w-[1440px] px-4 lg:px-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white md:text-3xl">Anime</h1>
          <p className="mt-1 text-sm text-muted">Discover the best anime series and movies</p>
        </div>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-4">
          <div className="flex flex-wrap gap-1.5">
            ...
            {GENRES.map((g) => (
              <a
                key={g}
                href={`/anime?genre=${encodeURIComponent(g)}&sort=${sort}`}
                className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
                  genre === g
                    ? 'bg-accent text-white'
          ...
          <div className="flex items-center gap-2">
            ...
            {SORT_OPTIONS.map((opt) => (
              <a
                key={opt.value}
                href={`/anime?genre=${genre}&sort=${opt.value}`}
                className={`rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors ${
                  sort === opt.value ? 'bg-accent text-white' : 'text-muted hover:text-white'
                }`}
              >
                {opt.label}
              </a>
        ...
        >
          <AnimeGrid genre={genre} sort={sort} page={params.page} />
        </Suspense>
      </div>
    </div>
  );
