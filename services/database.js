...
async function fullPushToMongo() {
  ...
  // Push ALL episodes
  const epR = db.exec("SELECT * FROM episodes");
  if (epR.length > 0 && epR[0].values.length > 0) {
    const allEps = epR[0].values.map(v => rowToObj(epR[0], v));
    await mongo.fullSyncEpisodes(allEps);
  }

  // Push ALL watchlist items
  const wlR = db.exec("SELECT * FROM watchlist");
  if (wlR.length > 0 && wlR[0].values.length > 0) {
    const allWl = wlR[0].values.map(v => rowToObj(wlR[0], v));
    await mongo.fullSyncWatchlist(allWl);
  }

  // Push ALL continue watching
  const cwR = db.exec("SELECT * FROM continue_watching");
  if (cwR.length > 0 && cwR[0].values.length > 0) {
    const allCw = cwR[0].values.map(v => rowToObj(cwR[0], v));
    await mongo.fullSyncContinueWatching(allCw);
  }
}
...
const users = {
  ...
  create({ name, email, password = '', google_id = '', role = 'user', avatar = '', plan = 'free' }) {
    ...
    if (user) getMongo()?.syncUser(user).catch(e => console.error("[MongoDB] user sync failed:", e.message));
    return user;
  },
  updateLastActive(id) {
  ...
  delete(id) {
    ...
    saveNow();
    getMongo()?.deleteUser(id).catch(e => console.error("[MongoDB] user delete failed:", e.message));
  }
};
...
const episodes = {
  ...
  upsert(contentId, data) {
    ...
    getMongo()?.syncEpisode(epData, contentId).catch(e => console.error("[MongoDB] episode sync failed:", e.message));
  }
};
...
const watchlist = {
  ...
  add(userId, contentId, type = 'watchlist') {
    ...
    saveNow();
    getMongo()?.syncWatchlist(userId, contentId, type, 'add').catch(e => console.error('[MongoDB] watchlist sync failed:', e.message));
  },
  remove(userId, contentId, type = 'watchlist') {
    ...
    saveNow();
    getMongo()?.syncWatchlist(userId, contentId, type, 'remove').catch(e => console.error('[MongoDB] watchlist sync failed:', e.message));
  },
...
const continueWatching = {
  ...
  upsert(userId, tmdbId, type, title, poster, genre, duration, progress) {
    ...
    saveNow();
    getMongo()?.syncContinueWatching(userId, { tmdb_id: tmdbId, type, title, poster, genre, duration, progress }, 'upsert').catch(e => console.error('[MongoDB] CW sync failed:', e.message));
  },
  remove(userId, tmdbId) {
    db.run("DELETE FROM continue_watching WHERE user_id = ? AND tmdb_id = ?", [userId, tmdbId]);
    saveNow();
    getMongo()?.syncContinueWatching(userId, { tmdb_id: tmdbId }, 'remove').catch(e => console.error('[MongoDB] CW sync failed:', e.message));
  }
};
...
const payments = {
  add(userId, amount, plan, method = 'UPI', status = 'completed') {
    ...
    saveNow();
    getMongo()?.syncPayment({ user_id: userId, amount, plan, method, status, transaction_id: txId }).catch(e => console.error("[MongoDB] payment sync failed:", e.message));
  },
  all() {
...
const comments = {
  ...
  add(userId, contentId, text) {
    ...
    saveNow();
    const item = this.findById(id);
    if (item) getMongo()?.syncComment(item).catch(e => console.error('[MongoDB] comment sync failed:', e.message));
    return item;
  },
  findById(id) {
  ...
  delete(id, userId) {
    ...
    saveNow();
    getMongo()?.deleteComment(id).catch(e => console.error('[MongoDB] comment delete failed:', e.message));
  },
  count(contentId) {
...
const logs = {
  add(type, message, admin = 'System') {
    ...
    saveNow();
    getMongo()?.logActivity(type, message, null, { admin }).catch(e => console.error("[MongoDB] log sync failed:", e.message));
  },
  all(limit = 100) {
...
async function restoreFromMongo() {
  ...
  try {
    ...
    // Also restore episodes — use contentSqliteId (stored in MongoDB)
    ...
    if (mongoEpisodes.length > 0) {
      ...
      for (const ep of mongoEpisodes) {
        ...
        // Find content by contentSqliteId (primary) or fallback to title lookup
        const cItem = ep.contentSqliteId ? content.findById(ep.contentSqliteId) : null;
        if (!cItem) continue;
        // Check if episode already exists in SQLite
        const existing = db.exec("SELECT id FROM episodes WHERE content_id=? AND number=? AND season=?",
          [cItem.id, ep.number || 1, ep.season || 1]);
      ...
      if (epRestored > 0) {
        saveNow();
        console.log(`[Restore] Restored ${epRestored} episodes from MongoDB`);
      }
    }

    // Restore watchlist from MongoDB
    const mongoWatchlist = await mdb.collection('watchlist').find({}).toArray();
    if (mongoWatchlist.length > 0) {
      let wlRestored = 0;
      for (const w of mongoWatchlist) {
        if (!w.userId || !w.contentId) continue;
        const existing = db.exec("SELECT id FROM watchlist WHERE user_id=? AND content_id=? AND type=?",
          [w.userId, w.contentId, w.type || 'watchlist']);
        if (existing.length > 0 && existing[0].values.length > 0) continue;
        db.run(
          `INSERT OR IGNORE INTO watchlist (user_id, content_id, type) VALUES (?, ?, ?)`,
          [w.userId, w.contentId, w.type || 'watchlist']
        );
        wlRestored++;
      }
      if (wlRestored > 0) {
        saveNow();
        console.log(`[Restore] Restored ${wlRestored} watchlist items from MongoDB`);
      }
    }

    // Restore continue_watching from MongoDB
    const mongoCW = await mdb.collection('continue_watching').find({}).toArray();
    if (mongoCW.length > 0) {
      let cwRestored = 0;
      for (const cw of mongoCW) {
        if (!cw.userId || !cw.tmdbId) continue;
        const existing = db.exec("SELECT id FROM continue_watching WHERE user_id=? AND tmdb_id=? AND type=?",
          [cw.userId, cw.tmdbId, cw.type || 'movie']);
        if (existing.length > 0 && existing[0].values.length > 0) continue;
        db.run(
          `INSERT OR IGNORE INTO continue_watching (user_id, tmdb_id, type, title, poster, genre, duration, progress)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [cw.userId, cw.tmdbId, cw.type || 'movie', cw.title || '', cw.poster || '',
           cw.genre || '', cw.duration || '', cw.progress || 0]
        );
        cwRestored++;
      }
      if (cwRestored > 0) {
        saveNow();
        console.log(`[Restore] Restored ${cwRestored} continue watching items from MongoDB`);
      }
    }
    ...
    if (mongoPayments.length > 0) {
      ...
      for (const p of mongoPayments) {
        // Match by userId (sqliteId) first, then fallback to userEmail
        let u = null;
        if (p.userId) u = users.findById(p.userId);
        if (!u && p.userEmail) u = users.findByEmail(p.userEmail);
        if (u) {
          const txId = p.transactionId || 'TXN' + Date.now().toString(36).toUpperCase();
          const existing = db.exec("SELECT id FROM payments WHERE transaction_id=?", [txId]);
          if (existing.length > 0 && existing[0].values.length > 0) continue;
          db.run(
      ...
      if (payRestored > 0) {
        saveNow();
        console.log(`[Restore] Restored ${payRestored} payments from MongoDB`);
      }
    }
...
const userLikes = {
  ...
  toggle(userId, contentId, type) {
    ...
    if (existing === type) {
      ...
      getMongo()?.deleteUserLike(userId, contentId).catch(e => console.error('[MongoDB] like sync failed:', e.message));
    } else {
      db.run(`DELETE FROM user_likes WHERE user_id=? AND content_id=?`, [Number(userId), Number(contentId)]);
      db.run(`INSERT INTO user_likes (user_id, content_id, type) VALUES (?, ?, ?)`, [Number(userId), Number(contentId), type]);
      getMongo()?.syncUserLike({ user_id: Number(userId), content_id: Number(contentId), type }).catch(e => console.error('[MongoDB] like sync failed:', e.message));
    }
    saveNow();
...
module.exports = {
  init, getDb, save, saveNow, getMongo,
